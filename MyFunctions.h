...
void takeReadings(int (&sum)[14], int(&min)[14], int(&max)[14], int(&count)[14], int day) { // will need to have the arrays passed to this maybe
	int currentcount = 0;
	int currentmin = 2001;
	int currentmax = 0;
	string bloodsugar("");
...
void getWeekly(int(&sum)[14], int(&min)[14], int(&max)[14], int(&count)[14], int week) {
	int thesum=0;
	int themin=2001;
	int themax=0;
	int thecount=0;
	...
	cout << "based on a total of " << thecount << " valid readings." << endl << endl; 
